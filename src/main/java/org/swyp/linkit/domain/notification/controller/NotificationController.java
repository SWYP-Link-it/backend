...
public class NotificationController {
    ...
    @Operation(
            ...
            description = "요청 관리 탭, 받은 요청, 보낸 요청, 메시지 탭의 미읽음 알림 개수를 조회합니다. " +
                    "messageTabCount는 채팅 메시지 알림(CHAT_MESSAGE) 미읽음 총합이며, 상단 네비게이션 배지 표시에 활용합니다."
    )
    ...
    @Operation(
            ...
            description = "사용자의 전체 알림 목록을 최신순으로 조회합니다. " +
                    "미읽음 알림 전체 + 읽은 알림(7일 이내) 순으로 반환됩니다. " +
                    "CHAT_MESSAGE 타입 알림은 채팅 수신 시 자동 생성되며, 메시지: '{발신자}님에게 메시지 요청이 왔습니다.' 형식으로 표시됩니다. " +
                    "refId는 채팅방 ID(chatRoomId)이므로, 알림 클릭 시 해당 채팅방으로 이동할 수 있습니다."
    )
    @ApiErrorExceptionsExample(NotificationExceptionDocs.class)
    ...
    @Operation(
            ...
            description = "메시지 목록(채팅방 리스트) 페이지에 진입했을 때 호출합니다. " +
                    "모든 CHAT_MESSAGE 알림을 읽음 처리하여 메시지 탭의 배지를 초기화합니다."
    )
    @ApiErrorExceptionsExample(NotificationExceptionDocs.class)
    ...
    @Operation(
            ...
            description = "특정 채팅방에 진입했을 때 호출합니다. 해당 채팅방(chatRoomId)의 CHAT_MESSAGE 알림을 읽음 처리합니다. " +
                    "WebSocket /enter 이벤트 시 서버 내부에서도 자동 처리되므로, REST API 직접 호출은 HTTP 채팅방 진입 경로에서 사용합니다."
    )
    @ApiErrorExceptionsExample(NotificationExceptionDocs.class)
