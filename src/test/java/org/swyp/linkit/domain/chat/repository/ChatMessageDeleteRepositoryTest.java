...
class ChatMessageDeleteRepositoryTest {
    ...
    void setUp() {
        ...
        chatRoom = chatRoomRepository.save(ChatRoom.create(mentor, mentee));

        message1 = chatMessageRepository.save(
                ChatMessage.create(chatRoom, mentor, SenderRole.MENTOR, "첫 번째 메시지"));
        message2 = chatMessageRepository.save(
                ChatMessage.create(chatRoom, mentee, SenderRole.MENTEE, "두 번째 메시지"));
        message3 = chatMessageRepository.save(
                ChatMessage.create(chatRoom, mentor, SenderRole.MENTOR, "세 번째 메시지"));
    }
    ...
    class SaveChatMessageDelete {
        ...
        void save() {
            // given
            ChatMessageDelete messageDelete = ChatMessageDelete.create(message1, mentor.getId());
            ...
            assertThat(savedDelete.getId().getUserId()).isEqualTo(mentor.getId());
        ...
        void save_mentee() {
            // given
            ChatMessageDelete messageDelete = ChatMessageDelete.create(message1, mentee.getId());

            // when
            ChatMessageDelete savedDelete = chatMessageDeleteRepository.save(messageDelete);

            // then
            assertThat(savedDelete.getId().getUserId()).isEqualTo(mentee.getId());
        }
    }
    ...
    class FindChatMessageDelete {
        ...
        void findById() {
            ...
            ChatMessageDeleteId id = new ChatMessageDeleteId(message1.getId(), mentor.getId());
        ...
        void findDeletedMessageIdsByUserId() {
            // given
            chatMessageDeleteRepository.save(ChatMessageDelete.create(message1, mentor.getId()));
            chatMessageDeleteRepository.save(ChatMessageDelete.create(message2, mentor.getId()));
            // message3은 삭제하지 않음

            // when
            List<Long> deletedMessageIds = chatMessageDeleteRepository.findDeletedMessageIdsByUserId(mentor.getId());
    ...
    class ExistsChatMessageDelete {
        ...
        void existsById_ChatMessageIdAndId_UserId_otherUserDeleted() {
            // given
            chatMessageDeleteRepository.save(ChatMessageDelete.create(message1, mentee.getId()));
    ...
    class MultiUserDelete {
        ...
        void bothUsersCanDelete() {
            // given
            ChatMessageDelete mentorDelete = ChatMessageDelete.create(message1, mentor.getId());
            ChatMessageDelete menteeDelete = ChatMessageDelete.create(message1, mentee.getId());
        ...
        void independentDeleteLists() {
            ...
            chatMessageDeleteRepository.save(ChatMessageDelete.create(message2, mentee.getId()));
            chatMessageDeleteRepository.save(ChatMessageDelete.create(message3, mentee.getId()));

            // when
            List<Long> mentorDeletedIds = chatMessageDeleteRepository.findDeletedMessageIdsByUserId(mentor.getId());
            List<Long> menteeDeletedIds = chatMessageDeleteRepository.findDeletedMessageIdsByUserId(mentee.getId());
